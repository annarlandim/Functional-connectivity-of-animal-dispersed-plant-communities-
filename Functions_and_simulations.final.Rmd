...
We used empirical trait data from plant and bird communities in Manú Biosphere Reserve, Peru.
...
Using the trait values and abundances from our simulated plant and bird communities, we built seed-dispersal networks to define the vectors of seed dispersal in the source patch. We varied network specialization levels to test its effect on plant functional connectivity.
...
# since abundances have decimals, the total number of interaction events is a bit below the sum of species abundances
```

**(VII) Calculate reachable patches**
...
Here we simulate plant community assembly at the target patch, with iterations sequentially increasing plant richness form 0 to 50 plant species in the target patch. Then, we estimate the relative attractiveness of the target patch at each addition of plant species. Given that each plant species produced a fiyed number of fruits in our model, these simulations also varied resource abundance in the target patch. The relative attractiveness value was used to estimate the probability of bird movement direction to the target patch.

**(VIII) Make resource communities in the target patch**
...
# Simulating plant communities at target patch

rec_webs_2 <- r_webs_rand(specsparlevel, Nplant, Nbird, Nrep, rec_webs_p_2)

# Calculate total plant abundance in the target patch plant communities
...
seeds_reach_prob_webs_2 <- reach_prob_webs(recipient_webs = rec_webs_2, specsparlevel = specsparlevel, patches = patches, obs = obsperplant*YES_pl_freq, Nplant = Nplant, Nrep = Nrep,
...
# Add iteration number, abundance and diversity in the target patch to the data frame:
...
```{r}
write.csv(disp_dim_2, "data_plotting.csv")
```
